...
public class BaseSystemHandleAdapter extends ElSystemHandleAdapter {

 
	public BaseSystemHandleAdapter(Map<String, Object> adapterInfoMap) {
		super(adapterInfoMap);
	}

	@Override
	public void preHandle(HttpServletRequest request, String inputData) throws UserException {
		AppLog.debug("[BaseSystemHandleAdapter-preHandle]...");
		...
		try{
			...
			new BaseAuthProcess().checkAuth(request, svcId, inputData);			
	...
	public void postHandle(ElHeader elHeader, UserHeader userHeader, Object[] serviceParams, Model resultModel) {
		AppLog.debug("[BaseSystemHandleAdapter-postHandle]...");
		...
		if( userHeader != null && userHeader instanceof BaseUserHeader ) {
			BaseUserHeader siteUserHeader = (BaseUserHeader)userHeader;
