...
public class BaseUserHeaderSetAdapter extends UserHeaderSetAdapter{

	public BaseUserHeaderSetAdapter(Map<String, Object> adapterInfoMap) {
		super(adapterInfoMap);
	}

	public void setInputUserHeader(UserHeader userHeader) throws AdapterException {
		BaseUserHeader inputUserHeader = (BaseUserHeader) userHeader;

		if (inputUserHeader != null) {
			// Input 에서 들어온 헤더 값으로 Context 의 UserHeader 값을 설정하여 Controller 로
			// 전달한다.( 필요한 값만 - 변조 위험 항목 제외 )
			BaseUserHeader contextUserHeader = null;
			try {
			    contextUserHeader = (BaseUserHeader) ControllerContextUtil.getUserHeader();
            }catch(Exception e){}
			...
			if (contextUserHeader != null) {
			    ...
			    contextUserHeader = new BaseUserHeader();
			}
		}
	}
	...
	public UserHeader getOutputUserHeader(String outputType, ElHeader elHeader, UserHeader userHeader, Object obj) throws AdapterException {
	    BaseUserHeader paramUserHeader = new BaseUserHeader();
	    try {
		    paramUserHeader = (BaseUserHeader)userHeader;
		} catch( Exception e) {}
